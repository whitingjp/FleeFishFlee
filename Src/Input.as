package Src
{
  ...
  public class Input
  {
    ...
    public function leftKey(held:Boolean=false):Boolean
    {
      var dict:Dictionary = held ? keyDownDictionary : keyPressedDictionary;
      return dict[KEY_LEFT] || dict[KEY_A]
    }

    public function rightKey(held:Boolean=false):Boolean
    {
      var dict:Dictionary = held ? keyDownDictionary : keyPressedDictionary;
      return dict[KEY_RIGHT] || dict[KEY_D];
    }

    public function upKey(held:Boolean=false):Boolean
    {
      var dict:Dictionary = held ? keyDownDictionary : keyPressedDictionary;
      return dict[KEY_UP] || dict[KEY_W];
    }

    public function downKey(held:Boolean=false):Boolean
    {
      var dict:Dictionary = held ? keyDownDictionary : keyPressedDictionary;
      return dict[KEY_DOWN] || dict[KEY_S];
    }

    public function anyKey(held:Boolean=false):Boolean
    {
      return leftKey(held) || rightKey(held) || upKey(held) || downKey(held);
    }
